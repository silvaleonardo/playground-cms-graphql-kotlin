...
@Service
class GetCommentsByUserIdService(private val commentRepository: CommentRepository) {

    fun execute(userId: Long, page: Int = 1, size: Int = 10): List<CommentDto> {
        val paging = if (page > 0)  (page -1) else 0
        val pageable: Pageable = PageRequest.of(paging, size)
        val comments = commentRepository.findByUserId(userId, pageable);
