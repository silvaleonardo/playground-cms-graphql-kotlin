...
@Service
class GetPagesByUserIdService(private val pageRepository: PageRepository) {

    fun execute(userId: Long, page: Int = 1, size: Int = 10): List<PageDto> {
        val paging = if (page > 0)  (page -1) else 0
        val pageable: Pageable = PageRequest.of(paging, size)
        val pages = pageRepository.findByUserId(userId, pageable)
